...
nnet_dir=${experiment_folder}/xvector_nnet_bea16k
#obs=xvectors_demencia_94abc_bea16k_special
obs=ATEST
...
if [ $stage -le -1 ]; then
  # make plp features and compute vad segmentations
  # for train
  for name in demencia; do
   steps/make_plp.sh --plp-config conf/plp.conf --nj 6 \
   --cmd "$train_cmd" data/${name} exp/make_plp $mfccdir
   utils/fix_data_dir.sh data/${name} # fix data directory
    # compute VAD
...
if [ $stage -le -2 ]; then
  ...
  utils/subset_data_dir.sh data/train_aug 1072 data/train_aug_19k
  utils/fix_data_dir.sh data/train_aug_19k

  # Make MFCCs for the augmented data.
  steps/make_mfcc.sh --mfcc-config conf/mfcc.conf --nj 6 --cmd "$train_cmd" \
    data/train_aug_19k exp/make_mfcc $mfccdir

  # Combine the clean and augmented TRAIN list.  This is now roughly
  # double the size of the original clean list.
  utils/combine_data.sh data/train_combined data/train_aug_19k data/train
...
local/nnet3/xvector/tuning/run_xvector_1a.sh --stage $stage --train-stage -1 \
 --data data/demencia  --nnet-dir $nnet_dir \
...
if [ $stage -le 7 ]; then
  ...
  # train data
  sid/nnet3/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 8G" --nj 8 --use-gpu false \
  ${nnet_dir} data/demencia $experiment_folder/${obs}
  # dev and test data
  echo "FINALIZED STAGE: $stage"
fi
...
if [ $stage -le -8 ]; then
  ...
  for name in train dev test1; do
      $train_cmd exp/xvectors_${name}/log/lda.log \
        ivector-compute-lda --total-covariance-factor=0.0 --dim=$lda_dim \
        "ark:ivector-subtract-global-mean scp:exp/xvectors_${name}/xvector.scp ark:- |" \
        ark:data/${name}/utt2spk exp/xvectors_${name}/transform.mat || exit 1;
  done
fi