...
experiment_folder=exp_23mf_no_augmented

mfccdir=`pwd`/${experiment_folder}/mfcc
vaddir=`pwd`/${experiment_folder}/mfcc

nnet_dir=${experiment_folder}/xvector_nnet_1a
obs=xvectors_pretrained

nj=1
stage=7

if [ $stage -le -1 ]; then
  utils/combine_data.sh data/train_dev_gen data/train data/dev data/emotion_aibo_gen
  utils/fix_data_dir.sh data/train_dev_gen
fi
...
if [ $stage -le 1 ]; then
  ...
  for name in class1_gen class2_gen class3_gen class4_gen class5_gen; do
...
# WHEN AUGMENTED: CHANGE DATA/{...} TO THE RESPECTIVE FOLDER GENERATED IN THE AUG STAGE (E.G. data/train_dev_combined)
if [ $stage -le 3 ]; then
  ...
  data/train_dev_gen data/train_dev_gen_combined_cmvn ${experiment_folder}/train_dev_gen_combined_cmvn
  utils/fix_data_dir.sh data/train_dev_gen_combined_cmvn
  echo "FINALIZED STAGE: $stage"
fi
...
# --data data/train_dev_gen_combined_cmvn --nnet-dir $nnet_dir \
...
#  for name in class1_gen class2_gen class3_gen class4_gen class5_gen; do
  for name in train dev test class1_gen class2_gen class3_gen class4_gen class5_gen; do
    sid/nnet3/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 8G" --nj ${nj} --use-gpu false ${nnet_dir} \
    data/${name} ${experiment_folder}/${obs}_${name}
  done
  # dev and test data
  echo "FINALIZED STAGE: $stage"
fi